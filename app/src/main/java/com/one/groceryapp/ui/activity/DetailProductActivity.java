...
public class DetailProductActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (featureProductModelList.get(position).getIsliked()) {
        ...
        binding.like.setOnClickListener(v -> {
            if (featureProductModelList.get(position).getIsliked()) {
                binding.like.setImageResource(R.drawable.heart);
                featureProductModelList.get(position).setIsliked(false);
            }else{
                binding.like.setImageResource(R.drawable.heartfill);
                featureProductModelList.get(position).setIsliked(true);
            }
        });


        binding.minus.setOnClickListener(v -> {
            int quantityOfItem = featureProductModelList.get(position).getProductNumber();
            if(quantityOfItem > 1){
                quantityOfItem--;
                binding.quantityOfItem.setText(String.valueOf(quantityOfItem));
                featureProductModelList.get(position).setProductNumber(quantityOfItem);
            }
        });
